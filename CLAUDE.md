...
│                                       # pdf-builder (native @react-pdf/renderer),
│                                       # retry (transient backoff for Gemini),
│                                       # filename, quotas,
...
| _(none for PDF)_                  | —        | PDF generation is **native** — `pdf-builder.tsx` renders PDFs in-process from the same structured data as the DOCX, so there's no PDF env var, no external service, no cost. The Drive-era `UPDRAFT_GOOGLE_SA_JSON_B64` was retired 2026-06-13 (Drive died with `403 storageQuotaExceeded`; the Sandbox+LibreOffice rebuild was abandoned as over-engineering). See `DECISIONS.md` 2026-06-13. |
