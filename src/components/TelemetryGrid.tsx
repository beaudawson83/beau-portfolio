...
export default function TelemetryGrid() {
  ...
  return (
    <section className="py-16 px-4 sm:px-6 lg:px-8 border-y border-[#1F1F1F]">
      <div className="max-w-7xl mx-auto">
        <motion.div
          ...
          className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-0 lg:divide-x lg:divide-[#1F1F1F]"
        >
          {metrics.map((metric, index) => (
            <motion.div
              ...
              className="lg:px-8 first:lg:pl-0 last:lg:pr-0 text-center lg:text-left"
            >
              {/* Label */}
              <div className="font-mono text-xs tracking-wider text-[#94A3B8] mb-2">
                {metric.label}
              </div>

              {/* Value */}
              <div className="font-mono text-2xl sm:text-3xl font-bold text-[#7C3AED] mb-2">
                <AnimatedValue value={metric.value} inView={isInView} />
              </div>

              {/* Context */}
              <div className="text-sm text-[#94A3B8] mb-1">{metric.context}</div>

              {/* Source */}
              <div className="font-mono text-xs text-[#94A3B8]/60">
                // {metric.source}
              </div>
            </motion.div>
          ))}
